...
import { cx, sortCx } from '@burro/shared/utils/cx';
import { isReactComponent } from '@burro/shared/utils/is-react-component';
...
export const Button = ({
  ...
  const commonClassName = cx(
    ...
    (loading || (isLink && (disabled || loading))) && 'pointer-events-none',
    isLink && (disabled || loading) && 'cursor-not-allowed',
    loading &&
      (showTextWhileLoading
  ...
  if (isLink && href) {
    const { href: hrefValue, onClick, ...restProps } = otherProps as LinkProps;

    return (
      <Link
        {...restProps}
        to={disabled ? '#' : (hrefValue as string)}
  ...
  return (
    <AriaButton
      ...
      type={(buttonProps.type || 'button') as 'button' | 'submit' | 'reset'}
      isPending={loading}
      isDisabled={disabled}
      {...commonProps}
    >
