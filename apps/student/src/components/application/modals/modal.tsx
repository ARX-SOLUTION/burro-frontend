...
export const ModalOverlay = (props: AriaModalOverlayProps) => {
  return (
    <AriaModalOverlay
      ...
      className={(state) =>
        cx(
          'fixed inset-0 z-50 flex min-h-dvh w-full items-end justify-center overflow-y-auto bg-black/40 px-4 pt-4 pb-[clamp(16px,8vh,64px)] outline-hidden sm:items-center sm:justify-center sm:p-8',
...
export const Modal = (props: AriaModalOverlayProps) => (
  <AriaModal
    ...
    className={(state) =>
      cx(
        'relative max-h-full w-full overflow-y-auto rounded-3xl bg-primary align-middle outline-hidden max-sm:rounded-xl',
...
export const Dialog = (props: AriaDialogProps) => (
  <AriaDialog {...props} className={cx('w-full outline-hidden', props.className)} />
);
