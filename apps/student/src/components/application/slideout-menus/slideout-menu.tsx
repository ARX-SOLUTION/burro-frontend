...
import { CloseButton } from '@burro/shared/components/base/buttons/close-button';
...
interface SlideoutMenuProps
  extends Omit<AriaModalOverlayProps, 'children'>, RefAttributes<HTMLDivElement> {
...
const Menu = ({ children, dialogClassName, ...props }: SlideoutMenuProps) => {
  return (
    <ModalOverlay {...props}>
      <Modal
        className={(state) =>
          cx(typeof props.className === 'function' ? props.className(state) : props.className)
        }
      >
        {(state) => (
