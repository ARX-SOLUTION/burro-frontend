import { createContext, type PropsWithChildren, type RefAttributes, useContext } from 'react';
import {
  Tag as AriaTag,
...
import { Dot } from '@burro/shared/components/foundations/dot-icon';
        ...
        <>
          <div className={cx('flex items-center gap-1', styles[context.size].content)}>
            {context.selectionMode !== 'none' && (
              <TagCheckbox size={context.size} isSelected={isSelected} isDisabled={isDisabled} />
            )}
          ...
          {(onClose || allowsRemoving) && (
            <TagCloseX size={context.size} onPress={() => id && onClose?.(id.toString())} />
          )}
        </>
      )}
    </AriaTag>
