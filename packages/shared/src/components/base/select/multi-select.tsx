...
import type { IconComponentType } from '@burro/shared/components/base/badges/badge-types';
import { HintText } from '@burro/shared/components/base/input/hint-text';
import { Label } from '@burro/shared/components/base/input/label';
import { Popover } from '@burro/shared/components/base/select/popover';
import { type SelectItemType, sizes } from '@burro/shared/components/base/select/select';
import { TagCloseX } from '@burro/shared/components/base/tags/base-components/tag-close-x';
import { useResizeObserver } from '@burro/shared/hooks/use-resize-observer';
...
export const MultiSelectBase = ({
  ...
  const selectedKeys = selectedItems.items.map((item: SelectItemType) => item.id);
...
const InnerMultiSelect = ({
  ...
  return (
    <div className="relative flex w-full flex-1 flex-row flex-wrap items-center justify-start gap-1.5">
      {!isSelectionEmpty &&
        comboBoxContext?.selectedItems?.items?.map((value: SelectItemType) => (
          <span
            key={value.id}
